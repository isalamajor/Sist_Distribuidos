...
{
	return (set_value_rpc_1_svc(argp->key, argp->value1, argp->V_value2, result, rqstp));
}
...
int
_modify_value_rpc_1 (modify_value_rpc_1_argument *argp, void *result, struct svc_req *rqstp)
{
	return (modify_value_rpc_1_svc(argp->key, argp->value1, argp->V_value2, result, rqstp));
}
...
claves_prog_1(struct svc_req *rqstp, register SVCXPRT *transp)
{
	union {
		...
		modify_value_rpc_1_argument modify_value_rpc_1_arg;
	...
	case modify_value_rpc:
		_xdr_argument = (xdrproc_t) xdr_modify_value_rpc_1_argument;
		_xdr_result = (xdrproc_t) xdr_int;
...
main (int argc, char **argv)
{
	...
	pmap_unset (CLAVES_PROG, CLAVES_VERSION);
	...
	if (!svc_register(transp, CLAVES_PROG, CLAVES_VERSION, claves_prog_1, IPPROTO_UDP)) {
		fprintf (stderr, "%s", "unable to register (CLAVES_PROG, CLAVES_VERSION, udp).");
		exit(1);
	}
	...
	if (!svc_register(transp, CLAVES_PROG, CLAVES_VERSION, claves_prog_1, IPPROTO_TCP)) {
		fprintf (stderr, "%s", "unable to register (CLAVES_PROG, CLAVES_VERSION, tcp).");
		exit(1);
	}
